...
func getAccessToken(accessTokenId string) (*accessToken, rest_errors.RestErr) {
	...
	if response == nil || response.Response == nil {
		return nil, rest_errors.NewInternalServerError("invalid restclient response when trying to get access token", nil /*errors.New("restclient error")*/)
	}
	...
	if err := json.Unmarshal(response.Bytes(), &at); err != nil {
		...
		return nil, rest_errors.NewInternalServerError("Error when trying to unmarshal access token response", nil /*errors.New("error procesing json")*/)
	}
	return &at, nil
}
